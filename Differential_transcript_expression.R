...
if (args[3]=="rsem"){
	s2c$path= paste0("/",args[1],"/Isoformes/",s2c$sample,".isoforms.results")
...
if (args[3]=="rsem"){
	txi <- tximport(s3c, tx2gene=tx2gene, lengthCol="length",txIdCol="transcript_id",  countsCol="expected_count" ,abundanceCol="TPM", geneIdCol="gene_id", txOut=TRUE)  #txout: False= gene True= transcrit
} else if (args[3]=="salmon"){
	txi <- tximport(s3c, type="salmon", tx2gene=tx2gene, txOut=T)
}
	print(names(txi))
	...
	print(head((normcounts)))
		...
		references=unique((tx2gene[,1:3]))
		 a=((tstrsplit(rownames(normcounts), "\\.")))
	  	normcounts$"target_id"=a[[1]]
		print(head(references))
		print(head(normcounts))
		 normcounts2=merge(y = normcounts, x = references[ , c("target_id","ext_gene")], by = "target_id", all.y=T)
		 print("normcount ok")

	write.table(unique(normcounts2), file=paste(ThePath,"/Normalized_Counts_isoforms.xls", sep=""),  sep='\t', quote=F, row.names=F)
	return(normcounts)
}
#-----------------------------------------------------------------------------------------------------------------------------#
#
...
pathways<- function(condition1,condition2, res1,selected, kegg.sets, org.eg.db){
	  ...
	  print(head(res2))
		...
		references=unique((tx2gene[,1:3]))
		print(head(references))
         	file_to_annoted=merge(y = res2, x = references[ , c("target_id","ext_gene")], by = "target_id", all.y=T)
	  ...
	  write.table(unique(file_to_annoted), file=paste("ALL_isoforms_",condition1,"_vs_",condition2,".xls", sep=""),sep="\t", row.names=F)
	 ...
	 print("ouuuuuuuuuuuuuuuuuuuuuuuuuuuui")
	  ...
	  lapply(keggres, head)
	  write.table(keggres$less, file = "iso_keggresless.txt",sep = "\t")
	  write.table(keggres$greater, file = "iso_keggresgreater.txt",sep = "\t")
	  write.table(keggres, file = "iso_keggres.txt",sep = "\t")
	 ...
	 if (length(b)>0){
	  ...
	  if (keggresId==TRUE){
			tmp = sapply(keggresids, function(pid) pathview(gene.data=foldchanges, pathway.id=pid, kegg.native =T, same.layer=F, species=idd, min.nnodes = 0, out.suffix=paste("iso__V2",condition1,"_vs_",condition2,"_upregulate_n",match(pid,keggresids), sep="")))
	  }
	  keggresId=FALSE
	  ...
	  if (keggresId==TRUE){
			  tmp = sapply(keggresids, function(pid) pathview(gene.data=foldchanges, pathway.id=pid, kegg.native =T, same.layer=F, species=idd, min.nnodes = 0, out.suffix=paste("iso_",condition1,"_vs_",condition2,"_downregulate_n",match(pid,keggresids), sep="")))
	 }
}
...
differentialexpression<-function(condition1,condition2){
	  ...
	  selected=selection_gene(res1, condition1, condition2, dds)
		...
		colnames(res1)[which(names(res1) == "target_id")] <- "target_id"
		references=unique((tx2gene[,1:3]))
         	res3=unique(merge(y = res1, x = references[ , c("target_id", "ext_gene")], by = "target_id", all.y=TRUE))
	

	  write.table(res3, file=paste("iso_No_cutoff_",condition1,"_vs_",condition2,".xls", sep=""),
		      sep="\t", quote=F, row.names=F)

	  write.table(res3[rownames(res1) %in% selected,], file=paste("iso_",condition1,"_vs_",condition2,".xls", sep=""),
		      sep="\t", quote=F, row.names=F)
